...
check_commit_match() {
    local remote_commit logged_commit
    ...
    # The log stores a short hash from `git log --oneline`; remote is full SHA.
    if [[ "$remote_commit" == "$logged_commit"* ]]; then
...
git log -1 --oneline > "$LLAMA_LOG_FILE"
